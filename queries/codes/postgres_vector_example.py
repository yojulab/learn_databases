...
def vector_crud_examples(conn):
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute("UPDATE documents SET embedding = %s WHERE id = 1;", (new_embedding,))
            print("ID 1 문서의 임베딩이 수정되었습니다.")

            new_metadata = {"category": "animal", "lang": "ko", "has_tail": True}
            cur.execute("UPDATE documents SET metadata = %s WHERE id = 1;", (Json(new_metadata),))
            print("ID 1 문서의 메타데이터가 수정되었습니다.")
            conn.commit()

            # 수정 확인
            cur.execute("SELECT content, metadata, embedding::text FROM documents WHERE id = 1;")
            ...
            # 4. DELETE: 데이터 삭제
            print("\n--- 4. DELETE: 데이터 삭제 ---")
            cur.execute("DELETE FROM documents WHERE id = 3;")
            print("ID 3 문서('바나나')가 삭제되었습니다.")
            conn.commit()
